function warpedSpks = getSqPSTHwarped(warpedSpks,Behaviour,Spikes)
%% make leverPSTH for self-initiated trials
% Pass through deconvolved calcium data to extract trial aligned data
% across all neurons
% 
Spks = warpedSpks.leverWarpedSpks_sorted;
[nTrials,nTmp,nNeurons] = size(Spks);
%%% MOVEMENT ALIGNED
win = floor(Behaviour.parameters.windowBeforeMI*30.048);
neurons = cell(nTrials,(length(unique(Behaviour.MIHitTrace(1).cleanedPullCounts))-1));
...
for n = 1:nTrials
    for sQN = 1:size(Spikes.PSTH.MIHit.pl,2) % substract 1 because we dont want zeros
         neurons{n,sQN} = squeeze(Spks(n,(pullIdx(sQN)-win):(pullIdx(sQN)+win),:))';
    end
end
...
for SqN = 1:size(neurons,2)
    ...
    warpedSpks.SQ_PSTH(SqN).win = win;
    warpedSpks.SQ_PSTH(SqN).MIHit.spks = neurons;
    warpedSpks.SQ_PSTH(SqN).MIHit.spkRate = output;
    warpedSpks.SQ_PSTH(SqN).MIHit.trialRate = make_nice_mean_raster(neurons(:,SqN)',5,0);
    warpedSpks.SQ_PSTH(SqN).Sequence = SqN;
end
