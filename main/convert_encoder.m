function encoder_data = convert_encoder(filename)
if nargin < 1
 [FileName,PathName,~] = uigetfile('*.wcp');
    filename = [PathName FileName];
end
out = import_wcp(filename);
dt = out.T(2)-out.T(1);
rotate_pulse = out.S{1,1};
pwd_max = max(rotate_pulse);
pwd_min = min(rotate_pulse);
pwd_threshold = pwd_max-pwd_min;
i = out.rec_index;
